...
def write_schema_file(schema, filename, output_dir=None):
    schemaJSON = ""
    ...
    f.close()
    print("JSON schema written to {filename}".format(filename=filepath))
...
if __name__ == "__main__":
  ...
  parser.add_argument('files', metavar='FILE', nargs='+', help='CRD files to process (local paths or URLs)')
  parser.add_argument('--output-dir', dest='output_dir', help='Directory to write JSON schema files')
              ...
              if "spec" in y and "versions" in y["spec"] and y["spec"]["versions"]:
                  for version in y["spec"]["versions"]:
                      if "schema" in version and "openAPIV3Schema" in version["schema"]:
                          ...
                          write_schema_file(schema, filename, args.output_dir)
